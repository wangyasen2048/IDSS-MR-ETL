...
//将频道数据上传到mysql
	public static void updateChannelToMysql(Set<channelBean> channelSet,
		...
		String call="{call basedata(?)}";
		
		String update="insert into  t_operator_channel  VALUES(?,?,?,?,?,?,?,?,?,default)"
				...
				for (channelBean c : channelSet) {
					...
					psUpdate.setString(9, operID+"0000");
				...
				try {
					conn.setAutoCommit(false);
					CallableStatement pscall = conn.prepareCall(call);
					pscall.setString(1, operID);
					boolean r=pscall.execute();
					conn.commit();
					log.info("执行"+operID+"的频道同步存储过程成功！"+r);
				} catch (SQLException e2) {
					e2.printStackTrace();	
					try {
						conn.rollback();
						log.warn("执行"+operID+"的频道同步存储过程失败！");
					} catch (SQLException e) {
						e.printStackTrace();
					}
				}
