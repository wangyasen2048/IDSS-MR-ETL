...
//			String localFileName = "E:/conf/"+cacheFileName;
	...
	private String parseVOD(String[] split,String inputDateID){
		...
		try {
			...
			String videoID=ReadCacheUtil.getMD5String(split[5]);
...
//			影片ID 影片名称  影片分集数
			result.append(videoID+OtherConstants.VERTICAL_DELIM+split[5]+OtherConstants.VERTICAL_DELIM+OtherConstants.VERTICAL_DELIM);
...
//			栏目名称      供应商ID   点播路径      节目总时长
			...
			ReadCacheUtil.getNewVODInfo(newCloumn, VODInfoList, videoID+delim+split[5], delim, delim,"010200");
...
//			直播频道换台方式   直播频道播放状态  逻辑频道号
			result.append(OtherConstants.VERTICAL_DELIM+OtherConstants.VERTICAL_DELIM+split[4]+OtherConstants.VERTICAL_DELIM);
//			频道名称    当前统一频道编号
			result.append(split[5]+OtherConstants.VERTICAL_DELIM+ReadCacheUtil.getUniqueChannel(newChannel,channelName,
					channelChannel, split[4],split[5]));
...
//			节目播出日期    节目播出时间   当前节目播放时间          节目播放状态
			result.append(OtherConstants.VERTICAL_DELIM+OtherConstants.VERTICAL_DELIM+OtherConstants.VERTICAL_DELIM+"1"+OtherConstants.VERTICAL_DELIM+ReadCacheUtil.getUniqueChannel(newChannel,channelName,
					channelChannel, split[4],split[5]));
		} catch (ParseException e) {
			return null;
		}
