...
public class event1 implements IETLOutput {

	private static Logger log = Logger.getLogger(event1.class);
	@Override
			...
			while(it.hasNext()){
				...
				try {
					...
					String recordDate=DateUtil.DATE_FORMATER.format(DateUtil.DATE_TIME_FORMATER.parse(split[0]));
						...
						String newrecord=operID+OtherConstants.VERTICAL_DELIM+recordDate+
								OtherConstants.VERTICAL_DELIM+uSplit[1]+OtherConstants.VERTICAL_DELIM+
								uSplit[0]+OtherConstants.VERTICAL_DELIM+time+OtherConstants.VERTICAL_DELIM+
						...
						try {
							mos.write(inputDateID, null, new Text(newrecord), inputDateID+OtherConstants.FILE_SEPARATOR+OutputPath.EVENT1_PREFIX+OutputPath.EVENT1+regionID);
						} catch (IOException e) {
