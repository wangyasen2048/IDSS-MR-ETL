package com.cucrz.idss.hadoop.etl.mapreduce.map.impl.chengdu;
		...
		if (split.length > 3) {
			if (split[0].length() == 16) {
				...
				String head = split[0] + OtherConstants.VERTICAL_DELIM
						+ "512000" + OtherConstants.TAB_DELIM + date + " "
		...
		// VOD节目
			...
			if (bizSplit.length == 7) {
				...
				ReadCacheUtil.getNewVODInfo(newCloumn, VODInfoList, bizSplit[0]
						+ delim + bizSplit[1], bizSplit[5] + delim + bizSplit[6],delim,"512000");
