...
public class DataEtlJob extends ControlledJob {
	...
	private static  String JarsPath="/user/oozie/rzcloud/lib/";
	/**
	 * 
	 * @param conf
	 * @throws IOException
...
//创建Job，指明各种mapreduce以及输入输出路径
public static Job createJob(Configuration conf) throws IOException,
			IdsshvException, URISyntaxException {
		FileSystem fs = FileSystem.get(conf);
		Job job = new Job(conf, "etlDataEtlJob_"+ conf.get("operatorID")+ "_"+
		      conf.get("inputDateID")+ "_"+ DateFormatUtils.format(System.currentTimeMillis(),"yyyyMMddHHmmssSSS"));
//		addDistributedJars(fs, conf, job);
		...
		if (isOpen) {
			// 添加distributedcache
			FileStatus[] files = fs.listStatus(new Path(conf.get("preCachePath")+ conf.get("operatorID")+ OtherConstants.FILE_SEPARATOR));
			...
			if (isOutputSampleUser.equals("true") && isSampleFileExits == false) {
				System.out.println("--------------------样本户列表文件不存在，程序退出-------------------------");
				return null;
			}
			...
			if (isOutputSampleUser.equals("true")) {
				System.out.println("-------------------分布式缓存已关闭，无法输出样本户，请检查配置------------------------");
				return null;
			}
		}
...
//			FileInputFormat.addInputPath(job,in);
			...
			Path[] paths = OperatorPath.getPath(operatorName,conf.get("operatorID"), conf.get("inputDateID"));
			for (Path p : paths) {
				FileStatus[] files = fs.listStatus(new Path(conf.get("fs.defaultFS")+ OtherConstants.FILE_SEPARATOR+ p.toString()));
...
//				FileInputFormat.addInputPath(job, p);
		...
		// 定义附加的输出文件
		MultipleOutputs.addNamedOutput(job, OutputPath.LAST_EVENT,TextOutputFormat.class, Text.class, Text.class);
		MultipleOutputs.addNamedOutput(job, OutputPath.WRONG,TextOutputFormat.class, Text.class, Text.class);
		MultipleOutputs.addNamedOutput(job, OutputPath.WHOLE,TextOutputFormat.class, Text.class, Text.class);
		MultipleOutputs.addNamedOutput(job, OutputPath.SAMPLE,TextOutputFormat.class, Text.class, Text.class);
		MultipleOutputs.addNamedOutput(job, OutputPath.NEW_CHANNLE,TextOutputFormat.class, Text.class, Text.class);
		MultipleOutputs.addNamedOutput(job, OutputPath.NEW_USER,TextOutputFormat.class, Text.class, Text.class);
		MultipleOutputs.addNamedOutput(job, OutputPath.NEWCLOUMN,TextOutputFormat.class, Text.class, Text.class);
		return job;
	}
	
//指明各种路径，然后设置conf，，调用createjob；
public static boolean run(Map<String, String> mapArgs) throws Exception {
		...
		//hdfs+文件路径+频道id+\+。。具体的文件路径
		String propertyPath=conf.get("fs.defaultFS")+JOB_PROPERTY + mapArgs.get("operatorID") + File.separator+ propertyFileName;
		//将 propertyPath下的文件以keyvalue形式存入mapargs
		...
		//具体路径
		String lastEvent=conf.get("fs.defaultFS")+ conf.get("outputPath")+ OtherConstants.FILE_SEPARATOR
...
//		boolean isFinished =true;
		if (isFinished) {
			...
			System.out.println("==========》 日期id为 "+ mapArgs.get("inputDateID")+ "----ETL处理成功！《==========");
			return true;
		} else {
			System.out.println("==========》 日期id为 "+ mapArgs.get("inputDateID")+ "----ETL处理失败！《==========");
			return false;
		}
	}
	
//增加jar包到每一个maptask或者reducetask
public static void addDistributedJars(FileSystem fs,Configuration conf,Job job) throws FileNotFoundException, IllegalArgumentException, IOException, URISyntaxException{
		Path jarPath=new Path(JarsPath);
		if(conf.get("preJarPath")!=null&&!conf.get("preJarPath").equals("")){
			jarPath=new Path(conf.get("preJarPath"));
		}	
		
		if(fs.exists(jarPath)){
		FileStatus[] files = fs.listStatus(jarPath);
		for (int i = 0; i < files.length; i++) {
			if (!files[i].isDir()) {
				String path = files[i].getPath().toString();
				int idx = path.lastIndexOf(OtherConstants.FILE_SEPARATOR);
